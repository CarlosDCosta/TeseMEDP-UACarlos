...
import math
commandId = 'Apply Material'
defaultVaseName = 'Vaso'
defaultDiameter1 = 5
defaultDiameter2 = 15
defaultDiameter3 = 5
defaultDiameter4 = 10
...
class VasoCommandExecuteHandler(adsk.core.CommandEventHandler):
    def __init__(self):
        super().__init__()
...
class VasoCommandDestroyHandler(adsk.core.CommandEventHandler):
    def __init__(self):
        super().__init__()
    def notify(self, args):
        try:
...
class VasoCommandCreatedHandler(adsk.core.CommandCreatedEventHandler):    
    def __init__(self):
        super().__init__()        
    def notify(self, args):
        try:
            ...
            onExecute = VasoCommandExecuteHandler()
            cmd.execute.add(onExecute)
            onExecutePreview = VasoCommandExecuteHandler()
            cmd.executePreview.add(onExecutePreview)
            onDestroy = VasoCommandDestroyHandler()
            ...
            inputs.addStringValueInput('vaseName', 'Nome do Vaso', defaultVaseName)
...
class Vaso:
    def __init__(self):
        self._vaseName = defaultVaseName
    ...
    # Loft
        ...
        # Obter a face superior do loft

        faceCol = adsk.core.ObjectCollection.create()
        faceCol.add(loft_feature.endFace)
        ...
        # Criar a concha
        shell_features = root_comp.features.shellFeatures
        shell_input = shell_features.createInput(faceCol)
        shell_input.insideThickness = adsk.core.ValueInput.createByReal(0.5)
        shell_feature = shell_features.add(shell_input)
        
        # Atualizar a visualização
        app.activeViewport.refresh()
        entities.append(shell_feature.bodies.item(0))
...
def run(context):
    try:
        ...
        cmdDef = commandDefinitions.itemById('Vaso')
        if not cmdDef:
            cmdDef = commandDefinitions.addButtonDefinition('Vaso',
                    'Criar o Vaso (do Caso de Estudo 2)',
                    'Criar o Vaso (do Caso de Estudo 2).',
                    '') # relative resource file path is specified

        onCommandCreated = VasoCommandCreatedHandler()
